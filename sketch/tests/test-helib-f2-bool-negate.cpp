...
int main(void) {
	...
	std::vector<ContextHElib_F2<bool>::Plaintext> plaintext_inputs = {input};
	std::vector<ContextHElib_F2<bool>::Ciphertext> ciphertext_inputs;
	
	for (ContextHElib_F2<bool>::Plaintext pt: plaintext_inputs)
	  ciphertext_inputs.push_back(ctx.encrypt(pt));
	
	std::vector<ContextHElib_F2<bool>::Ciphertext> ciphertext_outputs;
	using microsecond = std::chrono::duration<double, std::micro>;
	microsecond time = run_circuit(ciphertext_inputs, ciphertext_outputs);
	
	std::vector<ContextHElib_F2<bool>::Plaintext> plaintext_outputs;
