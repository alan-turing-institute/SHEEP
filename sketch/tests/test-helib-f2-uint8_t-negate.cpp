...
int main(void) {
	...
	typedef std::vector<ContextHElib_F2<uint8_t>::Plaintext> PtVec;
	...
	std::vector<ContextHElib_F2<uint8_t>::Plaintext> plaintext_inputs = {input};
	std::vector<ContextHElib_F2<uint8_t>::Ciphertext> ciphertext_inputs;
	
	for (ContextHElib_F2<uint8_t>::Plaintext pt: plaintext_inputs)
	  ciphertext_inputs.push_back(ctx.encrypt(pt));
	
	std::vector<ContextHElib_F2<uint8_t>::Ciphertext> ciphertext_outputs;
	using microsecond = std::chrono::duration<double, std::micro>;
	microsecond time = run_circuit(ciphertext_inputs, ciphertext_outputs);
	
	std::vector<ContextHElib_F2<uint8_t>::Plaintext> plaintext_outputs;
