...
int main(void) {
  ...
  using namespace Sheep::HElib;
	...
	ContextHElib<int8_t>::Plaintext pt_input = 55;
	ContextHElib<int8_t>::Ciphertext ct = ctx.encrypt(pt_input);
	long const_val = 2;
	ContextHElib<int8_t>::Ciphertext ct_out = ctx.MultByConstant(ct, const_val);
	ContextHElib<int8_t>::Plaintext pt_out = ctx.decrypt(ct_out);
