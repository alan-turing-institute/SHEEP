...
class ContextClear
    : public Context<PlaintextT, PlaintextT> {  // plaintext and ciphertext are
                                                // the same type
 public:
  ...
  Ciphertext Add(Ciphertext a, Ciphertext b) {
    if (std::is_same<Ciphertext, bool>::value) {
      ...
      typedef typename std::make_unsigned<
          typename CiphertextWrapper<Ciphertext>::type>::type uC;
  ...
  Ciphertext Maximum(Ciphertext a, Ciphertext b) { return (a >= b) ? a : b; }

  Ciphertext Not(Ciphertext a) { return !a; }
  ...
  Ciphertext Compare(Ciphertext a, Ciphertext b) { return (a > b); }

  Ciphertext Select(Ciphertext s, Ciphertext a, Ciphertext b) {
    return (s % 2) ? a : b;
  }

  Ciphertext MultByConstant(Ciphertext a, long b) { return Multiply(a, b); }

  Ciphertext AddConstant(Ciphertext a, long b) { return Add(a, b); }
};
