...
class ContextSeal : public Context<PlaintextT, seal::Ciphertext> {
 public:
  ...
  ContextSeal(
      ...
      long N = 2048)
      :  // This must be a power-of-2 cyclotomic polynomial described as a
         // string, e.g. "1x^2048 + 1"):
        m_N(N),
        m_security(security),
  ...
  void configure() {
    ...
    if (m_security == 128) {
      parms.set_coeff_modulus(
          seal::coeff_modulus_128(parms.poly_modulus().coeff_count() - 1));
    } else if (m_security == 192) {
      parms.set_coeff_modulus(
          seal::coeff_modulus_192(parms.poly_modulus().coeff_count() - 1));
    } else {
      throw std::invalid_argument(
          "Unsupported security value in ContextSeal, expected 128 or 129");
    }

    parms.set_plain_modulus(m_plaintext_modulus);
    m_context = new seal::SEALContext(parms);
    m_encoder = new seal::IntegerEncoder(
        m_context->plain_modulus());  // We default to an IntegerEncoder with
                                      // base b=2. TODO: include CRT and
                                      // fractional encoder
