...
class BaseContext {
 public:
  ...
  virtual std::vector<PlaintextT> eval_with_plaintexts(
      const Circuit& C, std::vector<PlaintextT> plaintext_inputs,
...
class Context : public BaseContext<PlaintextT> {
 ...
 public:
  ...
  // if a > b, returns a Ciphertext representation of 1, and a Ciphertext 0
  // otherwise.
  ...
  microsecond eval(
      const Circuit& circ, const InputContainer& input_vals,
  ...
  // 	auto run = std::bind(&Context::eval<std::list<Ciphertext>,
  // std::list<Ciphertext> >, this, circ, _1, _2,
  // std::chrono::duration<double, std::micro>(0.0)); 	return
  // CircuitEvaluator(run);
  // }

  // overload taking both durations and const_plaintext_inputs
  virtual std::vector<Plaintext> eval_with_plaintexts(
      const Circuit& C, std::vector<Plaintext> plaintext_inputs,
  ...
  // overload omitting durations only
  virtual std::vector<Plaintext> eval_with_plaintexts(
      const Circuit& c, std::vector<Plaintext> ptxts,
