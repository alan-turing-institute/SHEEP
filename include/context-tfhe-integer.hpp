...
template <typename T>
class ContextTFHE : public Context<T, CiphertextArrayTFHE<BITWIDTH(T)> > {
 ...
 public:
    ...
    for (int i = 0; i < BITWIDTH(Plaintext); i++) {
      for (int j = 0; j <= i; j++) {
        std::tie(sum_bit, carry_bit) =
            MultiplyBit(a[i - j], b[j], result[i], carry[j][i]);
  ...
  Ciphertext Select(Ciphertext s, Ciphertext a, Ciphertext b) {
    // bootsMUX(LweSample* result, const LweSample* a, const LweSample* b, const
    // LweSample* c, const TFheGateBootstrappingCloudKeySet* bk);
    Ciphertext result(parameters);
    for (size_t i = 0; i < BITWIDTH(Plaintext); i++)
