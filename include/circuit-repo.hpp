  ...
  Circuit create_circuit( Gate gate, int depth) {
    ...
    if (gate == Gate::Negate) {
      ...
      for (int i=1; i < depth; ++i) {
        Wire gate_output = C.add_assignment("output_"+ std::to_string(i), gate, output_wires[i-1]);
        output_wires.push_back(gate_output);
      }
      ...
      if (input_wires.size() > 1) {
        ...
        ///  loop over the rest of the inputs, combining an input with an output from the
        ///  previous operation and a select wire.    
        for (unsigned long i=2; i < input_wires.size(); ++i) {
          Wire gate_output = C.add_assignment("output_"+ std::to_string(i-1), gate,
                      select_wires[i-1], output_wires[i-2], input_wires[i]);      
          output_wires.push_back(gate_output);
        }
      }
