...
template <typename PlaintextT>
bool check_correct(std::vector<PlaintextT> test_results,
                   std::vector<PlaintextT> plaintext_results) {
...
template <typename PlaintextT>
void print_outputs(Circuit C, std::vector<PlaintextT> test_results,
                   std::vector<PlaintextT> cleartext_results,
...
template <typename T>
std::vector<T> match_inputs_to_circuit(Circuit C,
                                       std::map<std::string, T> inputs_map) {
...
bool benchmark_run(std::string context_name, std::string parameter_file,
  ...
  std::vector<PlaintextT> result_bench = test_ctx->eval_with_plaintexts(
      C, ordered_inputs, durations, eval_strategy);
  std::cout << " === Ran benchmark test. " << std::endl;
  test_ctx->print_parameters();
  test_ctx->print_sizes();
  std::vector<PlaintextT> result_clear = clear_ctx->eval_with_plaintexts(
      C, ordered_inputs, durations, eval_strategy);
