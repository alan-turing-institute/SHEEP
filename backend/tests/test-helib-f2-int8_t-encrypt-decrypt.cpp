...
void test_single(ContextHElib_F2<int8_t>& context) {
  ...
  //std::cout<"size of pt_new is "<<std::to_string((int)pt_new.size())<<std::endl;
  ...
  std::cout << std::to_string(pt_orig[2]) << " = " << std::to_string(pt_new[2]) << std::endl;
  std::cout << std::to_string(pt_orig[3]) << " = " << std::to_string(pt_new[3]) << std::endl;
