...
static void test_good(void) {
  ...
  Wire out = C.add_assignment("out", Gate::Add, std::string("in1"),
                              std::string("in2"));
  C.set_output(out);
}
...
static void test_multiple_assignment(void) {
  ...
  Wire in1 = C.add_input("in1");
  Wire in2 = C.add_input("in2");
  Wire w1 = C.add_assignment("reused_name", Gate::Add, std::string("in1"),
                             std::string("in2"));
  Wire w2 = C.add_assignment("reused_name", Gate::Add, std::string("in1"),
                             std::string("in2"));
}
...
int main(void) {
  ...
  try {
    ...
    std::cout
        << "test_multiple_assignment: Caught MultipleAssignmentError (ok).\n";
  }
