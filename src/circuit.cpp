  ...
  while (std::getline(stream, line)) {
    ...
    if (found != std::string::npos) {
      ...
      if (*token_iter == "CONST_INPUTS") {
        ...
        for (; token_iter != tokens.end(); ++token_iter) {
          try {
            ...
            std::cerr << "Error reading circuit: "
                      ...
                      << e.name
                      << "' has already been used to store a value (it must be "
                         "unique).\n";
            throw e;
          };
        }
        ...
        token_iter++;
        for (; token_iter != tokens.end(); ++token_iter) {
          // store outputs, to be set at the end, or an UndefinedVariableError
          // will be thrown.
        ...
        /// iterate over all tokens - the tokens before the gate name are
        /// inputs, those after are outputs.
          ...
          if (it != gate_name_map.end()) {
            ...
            try {
              ...
              std::cerr << "Error reading circuit: "
                        ...
                        << "' has already been used to store a value (it must "
                           "be unique).\n";
              throw e;
  ...
  for (std::pair<int, std::string> line_wire_pair : circuit_outputs) {
    try {
      ...
      std::cerr << "Error reading circuit: "
                ...
                << "' to be an output of the circuit as one with this name was "
                   "not defined.\n";
      throw e;
    }
  }
