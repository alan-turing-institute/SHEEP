...
template <typename PlaintextT>
std::vector<PlaintextT> SheepServer::make_plaintext_inputs() {
...
template <typename PlaintextT>
bool SheepServer::check_job_outputs(std::vector<PlaintextT> test_outputs,
...
void SheepServer::configure_and_run(http_request message) {
  ...
  std::vector<PlaintextT> plaintext_inputs =
  ...
  PlaintextT *outputs_shared = (PlaintextT *)mmap(
      NULL, n_outputs * sizeof(PlaintextT), PROT_READ | PROT_WRITE,
      MAP_ANONYMOUS | MAP_SHARED, 0, 0);
  ...
  if (child_pid == -1) {
    ...
    std::vector<PlaintextT> output_vals = context->eval_with_plaintexts(
        m_job_config.circuit, plaintext_inputs, const_plaintext_inputs, timings,
    ...
    if (!waitpid(child_pid, &status, WNOHANG)) {
      ...
      std::vector<PlaintextT> output_vals(outputs_shared,
                                          outputs_shared + n_outputs);
      ...
      std::vector<PlaintextT> clear_output_vals =
